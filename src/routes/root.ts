...
module.exports = async function (fastify: FastifyInstance, opts: {}) {
    fastify
        ...
        .get('/about', async function (request, reply) {
            return (reply as any).cookie('baz', 'baz')
                .view('about.ejs', {
                    data: "about page"
                });
        })
        .post('/c', async function (req: FastifyRequest, res) {
            ...
            if (bodys['image[]'].length) {
                ...
                fastify.upload(bodys['image[]'][0], "uploads/users/" + bodys['image[]'][0].name);
            }
